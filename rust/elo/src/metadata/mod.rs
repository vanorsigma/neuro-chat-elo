...
impl<M: AbstractMetadata> WithReceiver<M> {
    ...
    async fn get_metadata(&mut self, message: Message, sequence_no: u32) -> MetadataUpdate {
        self.metadata.get_metadata(message, sequence_no).await
    }
...
impl MetadataProcessor {
    pub async fn new(
        ...
        let basic_info = WithReceiver::new(
            basic_info::BasicInfo::new(
                seventv_client.clone(),
                twitch.clone(),
        ...
        let badges = WithReceiver::new(
            badges::Badges::new(&twitch).await,
            &broadcast_receiver,
            &mpsc_sender,
        );
        let special_role = WithReceiver::new(
            special_role::SpecialRole::new(),
            &broadcast_receiver,
            &mpsc_sender,
        );
        let chat_origin = WithReceiver::new(
            chat_origin::ChatOrigin::new(seventv_client, twitch, discord_client),
            &broadcast_receiver,
            &mpsc_sender,
        );
...
async fn calc_metadata<M: AbstractMetadata + Send + Sync + 'static>(
    ...
    */
    while let Ok((message, sequence_no)) = metadata.receiver.recv().await {
        let metadata_update = (*metadata).get_metadata(message, sequence_no).await;
