...
impl AdventuresDownloaderProxy {
    ...
    async fn get_rank_items_with_avatar(
        &self,
        rank_items: Vec<AdventuresRankItem>,
    ) -> Result<Vec<AdventuresRankItemWithAvatar>, anyhow::Error> {
    ...
    pub async fn get_ranks(
            ...
            |(game_map, response)| async {
                response
                    ...
                    .map(|rank_items| {
                        (
                            game_map,
                            self.get_rank_items_with_avatar(
        ...
        Ok(
            futures::future::try_join_all(responses_with_avatar.into_iter().map(
                |(game_map, response)| async {
                    response.await.map(|rank_item| (game_map, rank_item))
                },
            ))
            .await?
            .into_iter()
...
mod tests {
    ...
    async fn magical_test() {
        ...
        let ranks = super::AdventuresDownloaderProxy::new(token)
            .get_ranks()
            .await;
